...
- (void)viewDidLoad {
	...
	NSString *path2 = [[NSBundle mainBundle] pathForResource:@"whoElephant" ofType:@"mp3"];
	NSURL *url2 = [NSURL fileURLWithPath:path2];
	rightsn = [[AVAudioPlayer alloc] initWithContentsOfURL:url2 error:nil];
	

	
	self.navigationItem.title = @"여섯번째 마당";
...
- (IBAction)right:(id)sender{
	[rightsn play];
	whoexam7 *nextexam = [[whoexam7 alloc] init];
